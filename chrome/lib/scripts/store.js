...
const Store = {
  Load(callback) {
    ...
    if (config.email) {
      ...
      //Load the sites from the local storage
      config["sites"] = this.LoadCurrentTracking();
      callback(config);
    }
  },
  ...
  SaveUserSettings(userSettings, callback) {
    $.post(`${ServerBaseUrl}/users`, {
      ...
      //To save the sites in the local storage to the server
      let currentTracking = this.LoadCurrentTracking();
      if (currentTracking) this.SaveSites(currentTracking);
    });
  }
};
